...
var CoverageReporter = function (rootConfig, helper, logger, emitter) {
  ...
  this.onRunStart = function (browsers) {
    ...
    log.info('onRunStart completed')
  }

  this.onBrowserStart = function (browser) {
    log.info('onBrowserStart called')
    var startingMap = {}
    ...
    log.info('onBrowserStart completed')
  }
  ...
  this.onBrowserComplete = function (browser, result) {
    ...
    log.info('onBrowserComplete completed')
  }

  this.onSpecComplete = function (browser, result) {
    log.info('onSpecComplete called')
    var coverageMap = coverageMaps[browser.id]
    ...
    coverageMap.merge(result.coverage)
    log.info('onSpecComplete completed')
  }
  ...
  this.executeReport = async function (reporterConfig, browser) {
    log.info('generating report', reporterConfig, browser.name)
    const results = { exitCode: 0 }
    const coverageMap = coverageMaps[browser.id]
    if (!coverageMap) {
      log.warn(`no coverage for ${browser.name}`)
      return
    }
  ...
  this.onRunComplete = function (browsers) {
    ...
    log.info('onRunComplete completed')
